...
\documentclass[a4paper,openright,12pt,oneside]{report}
\usepackage[latin1]{inputenc}
\usepackage{graphicx} % graficos
\usepackage{multirow} % para las tablas
\usepackage{lettrine}%letras capitales
\usepackage{float}%orden [H]
...
\noindent Copyright \copyright{} 2016 Luis Ramírez de Diego\\ % Copyright notice
...
{\singlespacing
\begin{longtable}{|p{2cm}|p{3.3cm}|p{7.7cm}|}
\hline
...
\paragraph{PixieScript}

En la primera etapa del proyecto se lleva a cabo una búsqueda sistemática sobre el tema escogido.  En esta búsqueda es hallado el programa PixieScript\footnote{\url{https://sourceforge.net/projects/pixiescript/files/?source=navbar}}, que se utilizará como plantilla para el desarrollo del proyecto.  De esta forma se facilita la creación del código y la interfaz.
El código encontrado ha sido analizado, y debido a su adecuación con el objetivo del proyecto, se ha decidido tomarlo como base, ya que facilitaría el trabajo y cumplía muchos de los propósitos del proyecto. Aun así, se ha realizado una labor de análisis y modificación de la misma, ya que no respondía exactamente a lo buscado en este proyecto en concreto, y han tenido que ser añadidas nuevas herramientas que eran objeto de este proyecto.  La licencia de distribución de este software permite su adecuación a las necesidades del proyecto, añadiendo y modificando las funciones necesarias.

Pixiescript ofrece una interfaz relativamente sencilla para la ejecución de un buen número de ataques a redes WiFi y es relativamente sencillo incorporar nuevos ataques.
...
Este lenguaje ha sido utilizado, además de por sus múltiples ventajas ya citadas, por el hallazgo de PixieScript, ya que al estar realizado en este lenguaje, su aprendizaje nos facilitaba la posibilidad de modificar y poder utilizarlo como plantilla.
...
Se empleará este lenguaje de programación como herramienta de enlace entre el informe, generado en \LaTeX{} y las plantillas (\textit{templates}) empleando una biblioteca de python:  \textit{jinja2}.
...
\subsubsection{\LaTeX{}}
\LaTeX{} es un sistema de preparación de documentos. Sirve para organizar y preparar artículos de revistas, libros, cartas, revistas, tesis y cualquier tipo de documento. Se desarrolló con el fin de facilitar y mejorar los textos científicos, sobre todo los matemáticos, y que estos se puedan utilizar entre distintas plataformas. Con él se desarrolló la memoria del proyecto y la generación del informe. 
...
Para la creación de la aplicación ha sido utilizado el dispositivo Raspberry Pi 3 Modelo B, que como se aprecia en la imagen anterior (Figura \ref{fig:Ras}), mejora las características de sus predecesoras con un nuevo procesador más rápido integrado para incrementar su velocidad. Cuenta también con WiFi y Bluetooth BLE integrados para mejorar su funcionalidad y la capacidad de alimentar dispositivos más potentes mediante los puertos USB. Se optó por  este dispositivo por la facilidad que tiene a la hora de instalar un sistema operativo y la movilidad que proporciona, pudiendo transportarlo sin ningún tipo de impedimento. Contando por supuesto con su gran comunidad y las ventajas de esto a la hora de buscar información y de solucionar los posibles errores. 
\begin{figure}
  \centering
...
\begin{figure}
  \centering
   \includegraphics[height=0.9\textheight]{conectar.png}
...
Se descargó la versión para la Raspberry Pi 2/3 para instalarlo en la SD. \footnote{Recuperado de \href{http://docs.kali.org/kali-on-arm/install-kali-arm-on-a-beaglebone-black}{docs.kali.org}.}. Para hacer esto, previamente se instaló el sistema operativo Raspbian ya que cuenta con una incompatibilidad de la versión para la Raspberry Pi, y se debe cambiar la partición de arranque de la SD. En primer lugar se instala la versión Raspbian guardando la partición de arranque, y así se instala el sistema operativo Kali Linux.

Las instalaciones se pueden hacer de dos maneras dependiendo del sistema operativo que se disponga para la instalación.

\begin{enumerate}
	\item Desde Linux o Mac OS: Con la orden \texttt{dd if=kali-2.1.2-rpi2.img of=/dev/disk2}, siendo \texttt{/dev/disk2} el dispositivo de bloques correspondiente a la tarjeta SD y \texttt{kali-2.1.2-rpi2.img} la imagen del sistema operativo.  Se debe tener especial cuidado al utilizar esta orden, ya que se tiene que tener claro sobre qué dispositivo se vuelca información para que no se tenga problemas de borrado de disco.
...
El desarrollo de la aplicación que detecta las vulnerabilidades se llevó a cabo mediante la programación de la Raspberry Pi con lenguaje de programación bash (Ver código en ANEXO C).

\section[Interfaz gráfica]{Interfaz gráfica simple e intuitiva para la aplicación}
%%%%INTERFAZ%%%%%
Se pretende que la interfaz sea ejecutada por un comercial sin conocimientos informáticos previos, por ello se realiza un diseño sencillo de la misma.

\begin{figure}
  \centering
   \includegraphics[height=0.9\textheight]{interfaz.png}
...
La herramienta con la que se ha desarrollado la interfaz ha sido realizada en código bash.  Utiliza la biblioteca \textit{GTK dialog} para la gestión de pantalla, como interfaz gráfica. Pudiendo interactuar con diferentes botones que realizarán la tarea de análisis. 
Esto conformaba como objetivo inicial del proyecto, pero gracias a PixieSript esta tarea se ha podido llevar a cabo con mayor facilidad.

El código de Pixiescript ha sido analizado, y debido a su alineamiento con los objetivos del proyecto, se ha decidido tomarlo como base.  Aún así, se ha realizado una labor de análisis y modificación de Pixiescript, ya que no respondía exactamente a lo buscado en este proyecto, y han tenido que ser añadidas nuevas herramientas que eran objeto de este proyecto.

A continuación se ofrece un listado de aquellas funciones que han sido modificadas y adecuadas para el proyecto y aquellas que han sido creadas para funciones determinadas de las que el origen carecía.

Las funciones que han sido creadas o modificadas parcial o casi totalmente son las siguientes:
\begin{itemize}
	\item ScanearRedes()
	\item ProcesarDatosEscaneoAirodump()
	\item EsperarAcabarAircrack()
	\item BuscarClienteConectado()
	\item AircrackWEP()
	\item AircrackWPA()
	\item AnalizarPyrit()
...
Ademas, dentro de la interfaz PixieScript, tomada como base, se han añadido botones según necesidades del proyecto, y del mismo modo han sido eliminados los que para este caso no eran necesarios.
Los botones creados son los siguientes:
\begin{lstlisting}[label=nvinterfaz,caption=Botón aircrack]
	<button>
...
\begin{lstlisting}[label=nvinterfaz1,caption=Ataque WPA]
	<button>
		<input file>'$ICONO_START_ATACK'</input>
		<sensitive>false</sensitive>
...
\end{lstlisting}	
También, respecto a la interfaz, se ha añadido y adaptado la tabla principal donde se muestran las redes disponibles.
\begin{lstlisting}[label=nvinterfaz2,caption=Tabla de redes]
...
\begin{enumerate}
    \item Se inicia la aplicación \emph{SeguridadWifi}, se muestra la pantalla principal de la aplicación y se listan las interfaces WiFi disponibles para utilizar en el análisis de vulnerabilidades WiFi.
...
\item En esta pantalla se muestran las tarjetas WiFi disponibles y se activa el modo monitor de la tarjeta seleccionada.
...
\end{lstlisting}
        ...
        \item En la interfaz se presiona el botón \emph{ESCANEAR REDES} para buscar las redes que se encuentran a nuestro alcance en un tiempo determinado.
...
ScanearRedes() {
    ...
    for A in `seq 1 $TIEMPO_SCANEO` ; do
        ...
        if [ $A -eq $TIEMPO_SCANEO ]; then
            ...
            if [ -f  $ARCHIVO_DETENER_SCAN ] ; then
...
\begin{lstlisting}	[label=prueba2,caption=Listar Redes]
ListarTarjetasWifi() {   
    INTERFACES_MONITOR=`iwconfig --version | grep "Recommend" | awk '{print $1}' | grep mon`
    let CUANTAS=`echo $INTERFACES_MONITOR | wc -w`
    let CONT=1
...
\begin{lstlisting}[label=prueba3,caption=Aircrack WEP]
AircrackWEP() {
    echo "***************** Aircrack WEP******************"
    CambioMacRandom
...
Una vez detectado WPS en el punto de acceso se prueba mediante la herramienta \textit{reaver} si el punto es vulnerable. Ésta realizará un ataque de fuerza bruta sobre el punto de acceso, enviando distintos PIN hasta encontrar el que le devuelva la credencial de la red con cifrado WPA2.

\lstinputlisting[language=Bash,
                 label=prueba5,
                 caption=Ataque Reaver] {Atacarreaver().txt}

\paragraph{Pixiewps:}
Este ataque \textit{Pixiewps} es también conocido como \textit{polvo mágico}. Lo primero que hará es comprobar si el router al que se le realiza el ataque es vulnerable. Si así lo fuera, se podrá obtener la clave en segundos. Continuando los pasos de los ataques anteriores y utilizando la tarjeta en modo monitor y escaneando el tráfico, se atacará a una de las redes disponibles que disponga de WPS activado. Primero utilizaremos la orden \textit{reaver} para recoger la PKE (clave pública de la persona inscrita), PKR (clave pública del registrador), hash E-1 (hash  de la primera mitad del PIN WPS, por fuerza bruta) y hash E-2 (hash  de la segunda mitad del PIN WPS, por fuerza bruta), E-nonce/ R-nonce y la authkey. Una vez se obtenga toda la información, se podrá aplicar pixiewps, para reducir tiempos.
...
\begin{figure}
  \centering
   \includegraphics[width=\textwidth]{InformeWEP.png}
...
\begin{figure}
  \centering
   \includegraphics[width=\textwidth]{informeWPA.png}
  \caption{Informe WPA.}
  \label{fig:i}
\end{figure} 

Como en el caso anterior, el informe WPA se puede desglosar en varias secciones que generan un único informe.
Se utilizan los mismos programas y el mismo lenguaje de programación por lo que no se volverá a explicar de nuevo, pudiendo leerlo en el ANEXO C, si fuera necesario.
...
La seguridad en las redes inalámbricas debería ser un objetivo imprescindible para cualquier empresa, dando así la importancia que se merece la seguridad de la información.  La motivación de este proyecto, es conseguir concienciar al mayor número posible de empresas, del fácil acceso y los posibles perjuicios económicos e incluso jurídicos que le puede ocasionar un posible ataque.  Por ello, la importancia de hacer auditorias de forma periódica.

Como posibles estudios futuros, me gustaría seguir desarrollando la aplicación, ampliando el número de ataques y recogiendo mayor información en el informe automático.
...
\begin{itemize}
        ...
        \item Por muy simple que parezca, es importante cambiar los datos de acceso al router que vienen por defecto. No olvidar cambiar la contraseña y utilizar contraseñas de al menos 16 caracteres, empleando mayúsculas minúsculas y caracteres especiales. 
...
\includepdf[pages=-,frame,scale=0.8,clip,pagecommand={}]{datos-informe/INFORME/informe_WEP}

\chapter{Informe WPA}\label{anexo.B}
\includepdf[pages=-,frame,scale=0.8,clip,pagecommand={}]{datos-informe/INFORME/informe-WPA}

\chapter{Código}\label{anexo.C}
En el CD se incluye el código en dos archivos. En uno de ellos se describe las funciones empleadas en la aplicación (FUNCIONES-TFG) mientras que en SeguridadWiFi se describe el código utilizado para el desarrollo de la interfaz gráfica.
%\lstinputlisting[language=Bash,
        %         label=Seguridad,
     %            caption=SeguridadWiFi] {SeguridadWifi.txt}
%\lstinputlisting[language=Bash,
%		label=Funciones,
   %           caption=Funciones-TFG]{FUNCIONES-tesis.txt}
