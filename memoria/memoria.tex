...
Desde el inicio del nuevo siglo, el uso de las tecnologías forma parte de nuestra actividad cotidiana, lo que implica que cada vez se tiende más, y con mayor frecuencia, a compartir en red todo tipo de datos.
...
Desde que la tecnología forma parte de nuestra vida cotidiana, se encuentra cada vez más presente en las empresas, la pérdida o robo de información de datos electrónicos. Esto conlleva un importante agujero de seguridad, debido a una falta de previsión en la implantación de los métodos de seguridad.
...
\paragraph{Raspberry Pi}
 Raspberry Pi es un hardware que nos permite con gran facilidad automatizarlo y emplearlo para multiples fines. Se trata de un mini-ordenador  que se desarrolló en el Reino Unido, por un grupo del Computer Lab que empezó a preocuparse por el nivel de conocimientos sobre tecnologías informáticas con los que los estudiantes llegaban a la universidad.
...
La gran ventaja de este dispositivo radica en la gran comunidad que presentan, así dispone de mucha información y ayuda. En cuanto al sistema operativo, dispone de la gran ventaja que al instalarse en la tarjeta SD, se podrá contar con varios SO en distintas tarjetas. El más utilizado será ``Raspbian'' que se trata de un Debian(Linux) optimizado para este dispositivo.
La gran comunidad que se comento con anterioridad, ha permitido la interoperabilidad con otros productos. Se han creado periféricos específicos para Raspberry Pi (camaras, paneles tactiles con pantalla  TFT,etc). 



\subsubsection{Detalles del Software}
En cuanto al software se empleará una serie de programas informáticos, que harán posible la realización de este proyecto.
\paragraph{GNU/Linux}
...
\chapter{Detalles de configuración de la Raspberry Pi}

En el proyecto, utilizare la Raspberry Pi 3 Modelo B, que como podemos comprobar en la imagen anterior (Figura \ref{fig:Ras}), mejora las características de sus predecesoras con un nuevo procesador más rápido integrado para incrementar su velocidad. Cuenta también con funciones de WiFi y Bluetooth BLE para mejorar su funcionalidad y la capacidad de alimentar dispositivos más potentes mediante los puertos USB. Se ha utilizado este dispositivo por la facilidad que tiene a la hora de instalar un sistema operativo y la movilidad que te proporciona, pudiendo transportarlo sin ningún tipo de impedimento.
\begin{figure}[H]
  \centering
...
\section{Programación y funcionamiento de la Raspberry Pi}
Como ya se comento, la programación se ha desarrollado utilizando bash, 
