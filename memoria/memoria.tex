...
<<<<<<< HEAD
En la primera etapa del proyecto, en la que es llevada a cabo la investigación sobre el tema escogido, es hallado el programa PixieScript\footnote{\url{https://sourceforge.net/projects/pixiescript/files/?source=navbar}}, el cual se utilizara como plantilla para el desarrollo del proyecto, facilitando la creación del código y la interfaz, y adecuando lo hallado según las necesidades del proyecto, añadiendo y modificando las funciones.
...
Además, PixieScript cuenta con licencia Apache, la cual permite realizar modificaciones de software y distribuir las versiones modificadas del software bajo los términos de la licencia.
A continuación se detallara un listado de aquellas funciones que han sido modificadas y adecuadas para el proyecto y aquellas que han sido creadas para funciones determinadas de las que el origen carecía.
=======
En la primera etapa del proyecto se llevó a cabo una búsqueda sobre el tema escogido.  En esta búsqueda es hallado el programa PixieScript\footnote{\url{https://sourceforge.net/projects/pixiescript/}}, que se utilizará como plantilla para el desarrollo del proyecto.  De esta forma se facilitará la creación del código y la interfaz.  La licencia de distribución de este software permite su adecuación a las necesidades del proyecto, añadiendo y modificando las funciones necesarias.
>>>>>>> 9f3abe6741f614e7a241e016eea3e87d422b2de9
...
\begin{enumerate}
	\item Desde Linux o Mac OS: Con la orden \texttt{dd if=/Users/luis/Desktop/kali-2.1.2-rpi2.img of=/dev/disk2}, siendo \texttt{/dev/disk2} el dispositivo de bloques correspondiente a la tarjeta SD y \texttt{kali-2.1.2-rpi2.img} la imagen del sistema operativo.  Se debe tener especial cuidado al utilizar esta orden, ya que se tiene que tener claro sobre qué dispositivo se vuelca información para que no se tenga problemas de borrado de disco.
...
\includepdf[pages=-,frame,scale=0.8,clip,pagecommand={}]{informe_WEP}

\chapter{Informe WPA}\label{anexo.B}
\includepdf[pages=-,frame,scale=0.8,clip,pagecommand={}]{informe-WPA}

\chapter{Código}\label{anexo.C}
%\lstinputlisting[label=Seguridad,
   %               caption=SeguridadWifi]{SeguridadWifi}
%\lstinputlisting[language=Bash,
	%	label=SeguridadF,
      %         caption=Funciones]{FUNCIONES-tesis}
%\begin{lstlisting}[label=funciones,caption=Funciones-tesis]
