...
La fase de confidencialidad se encarga de que nadie que estuviera \textit{escuchando} y \textit{capturando} la transmisión  pueda entender o analizar la información, para ello dispone de varios elementos a tener en cuenta:
	...
	WEP permite utilizar claves de distintos tamaños:  64 bits (40 de la clave del usuario más 24 de los vectores de iniciación) o de 128(104 de la clave del usuario más 24 de los vectores de iniciación)
		\item IV, son los vectores de inicialización , son la parte dinámica de los \textit{keystream.} Cada trama lleva un IV distinto, aunque al ser generados de forma aleatoria no siempre es posible. Los IV ván en la parte que no se cifra de la trama WEP.
	\end{itemize}
\begin{figure}[H]
  \centering
...
``\textit{El pentesting no debería ser un proceso puntual que se contrata, sino un servicio de ataque 24x7 durante todo el ciclo de vida de tu sistema.}'' \footnote{ Recuperado de Un informático en el lado del mal. \url{http://www.elladodelmal.com/2013/05/pentesting-by-desing.html}}
...
\begin{itemize}
	\item Airmon-ng: Sirve para cambiar el modo de trabajo de la tarjeta inalámbrica, siempre y cuando el chipset lo permita. Este modo de trabajo cambiará a modo monitor.
...
\end{figure}
Las características principales del modelo de Raspberry Pi\footnote{Recuerado de \url{https://www.raspberrypi.org/wp-content/uploads/2016/03/pi3.jpg}} utilizados son las siguientes:
\begin{itemize}
...
Para conseguir incorporar el modo monitor se realizó una búsqueda completa en el mercado con varios posibles Chipset (Ralink RT3070, Ralink RT3572, Realtek 8187L, Ralink RT5370), empleando Atheros AR9271L \footnote{Pagina de compra de      \url{http://www.tienda-alfanetwork.com/AWUS036NHA-informacion.php}}, ya que no era excesivamente caro y proporcionaba una ganancia de 5 dBi y un rango de frecuencia de 2.412-2.483, y que cuenta con las siguientes características:
...
\section[Dotación de una interfaz gráfica]{Dotación a la aplicación de una interfaz gráfica simple e intuitiva.}
%%%%INTERFAZ%%%%%
...
%\fontsize{7pt}{7pt}=\selectfont
\begin{lstlisting}	[label=prueba2,caption=Escanear redes]
ScanearRedes() {
...
%\fontsize{7pt}{7pt}=\selectfont
\begin{lstlisting}	[label=prueba7,caption=PixieWPS]
AnalizarLogPixiewps(){
 ...
 do 
  ...
  then 
    ...
    then
	 ...
	 echo $ESSID"|"$BSSID"|"$PIN_WPS"|"$CLAVE_WPA"|"$fabricante"|"$modelo"|"$numero_de_modelo"|"$numero_de_serie"|"
	 $device_name >> $ARCHIVO_DATABASE_KEYS
    else
...
\begin{itemize}
	...
	\fontsize{7pt}{7pt}=\selectfont
\begin{lstlisting}	[label=informeWEP,caption=Sección WEP.py]
import jinja2
import sys, os
...
\end{lstlisting}
		...
		\begingroup{}[label=informeWEP1,caption=Sección WEP-Template]
		\fontsize{7pt}{7pt}=\selectfont
		...
		\begingroup{}[label=informeWEP2,caption=Sección WEP.tex]
		\fontsize{7pt}{7pt}=\selectfont
